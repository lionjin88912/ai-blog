...
```
python3 .agents/skills/persona-writer/scripts/detect_site.py <URL>
```
...
路徑:`.agents/skills/persona-writer/personas/<persona-slug>/wp-config.json`
內容:
```json
{
...
執行:
```
python3 .agents/skills/persona-writer/scripts/wp_oauth_setup.py <persona-slug>
```
...
```
python3 .agents/skills/persona-writer/scripts/wp_poster.py <persona-slug> "連線測試 - 可刪除" "<p>這是 Gemini 自動發送的測試文章,確認連線後可以刪除。</p>" draft
```
...
執行:
```
python3 .agents/skills/persona-writer/scripts/detect_site.py <使用者剛剛給的網址>
```
...
**先掃既有 draft**:在問使用者任何問題前,掃 `.agents/skills/persona-writer/personas/` 下**每個非 `_template`** 子資料夾的 `articles/draft-*.json`。
...
**先看開場訊息**:使用者一開口就把主題 + 關鍵字 + 人格都給齊了(例:「幫我用林太寫日月潭兩天一夜慢旅,關鍵字日月潭慢旅, 湖畔散策, 私房茶席」),**不要再問一次** A-1 的兩格表單,直接抽出來進 A-2。**但仍要在 A-4 預告時把抽到的值覆述一次給使用者確認**。
...
掃 `.agents/skills/persona-writer/personas/` 排除 `_template`:
- **只有一位人格 + 使用者沒指定別人**:直接帶他用該位(例:目前只有林太就用林太),口頭跟使用者預告「我會用『林太』寫,沒問題吧?」
- **使用者指定的人格存在**(例「用王老闆寫」且 `mr-wang-foodie/` 確實在):照他說的
- **使用者指定的人格不存在**(例「用王老闆寫」但 `personas/` 底下找不到對應資料夾):
  > 我找不到「王老闆」這位寫手 😅 目前可用的有:<列出 personas/ 排除 _template 的所有 display_name + topic>
  >
  > 你要從現有的挑一位,還是要先新增「王老闆」(我可以帶你做,大約 5-7 分鐘)?
  - 使用者挑現有 → 用挑的那位繼續 A-3
  - 使用者要新增 → **先在對話中保留當前主題 + 關鍵字**(口頭記下,例:「好,先記著你想寫『日月潭兩天一夜慢旅』+ 關鍵字『日月潭慢旅, 湖畔散策, 私房茶席』,新增完王老闆我們直接接著寫」),跳到模組 5 走新增人格流程。模組 5 Step E 完成後問「想立刻試一篇嗎?」,使用者說好就用剛才記下的主題 + 關鍵字回模組 2,從 **新人格的 Step 1A-3** 接續(不要再問一次主題)
- **多位人格 + 使用者沒指定**:列出所有人格的 `display_name` + `topic`,讓他挑

**A-3. 確認該人格的 WordPress 已設定**

檢查 `.agents/skills/persona-writer/personas/<slug>/wp-config.json` 是否存在。
...
執行 `write_file`:
- **路徑**:`.agents/skills/persona-writer/personas/<persona-slug>/articles/draft-<YYYYMMDD>-<HHMM>-<topic-slug>.json`
...
```
[1] AI 預告(粗體 step 標題 + 1 行「我為什麼這樣選」reasoning)
    例:**Step 3 / H1 標題**
        把焦點關鍵字「日月潭慢旅」擺最前面、加副標串起「湖畔散策、私房茶席」, 結尾配上部落格名收尾 ──

[2] AI 給「一個」建議(粗體或明確標出來)
    例:**「日月潭兩天一夜慢旅全攻略｜林太的湖畔散策、私房茶席與熟齡住宿心法 | 林太的慢活旅行誌」**
...
>
> **為什麼要加 reasoning?** dogfood 觀察:直接 show 結果問「好不好」,使用者不知道在第幾步、不知道 AI 為什麼這樣選,容易錯愕。1 行 reasoning 把判斷依據攤開,使用者比較容易判斷接不接受。
...
2. 重新生成,**把使用者的修改方向直接帶進這次的重生**
3. 再次走「預告 → 給建議 → 問意見」三段式
4. 同階段重生 **上限 5 次**(見下)

#### D. 中斷指令(放棄 / 換人格 / 跳出)

**語意特徵**:使用者要中止當前流程,不是針對「這一階段的建議」做反應。包含「放棄」「不寫了」「算了」「換 OOO 寫」「先停一下」「我想改成寫別的主題」。
**動作**:**不要**走 A/B/C,跳到 **模組 2 結尾的「錯誤與中斷情境彙整」表格**對應列處理:
- 放棄 → 二次確認 → `os.remove draft-*.json`
- 換人格 → 二次確認(舊內容會作廢)→ 刪舊 draft → 走新的 Step 1A
- 換主題(同人格)→ 視為「放棄這篇 + 開新的」二次確認後處理
...
從 draft.json 讀 `topic` / `keywords` / `research` + 該人格的 SEO 風格,生 **1 個** H1 標題(50-60 字元,**首位放焦點關鍵字**,用 `｜` 分隔 sub。不要以寫手或人格名稱結尾)。

範例:`日月潭兩天一夜慢旅全攻略｜林太的湖畔散策、私房茶席與熟齡住宿心法`

呈現給使用者(三段式 + reasoning):

```
**Step 3 / H1 標題**
把焦點關鍵字「日月潭慢旅」擺最前面、加副標串起「湖畔散策、私房茶席、熟齡住宿」, 結尾配上部落格名收尾 ──

**「日月潭兩天一夜慢旅全攻略｜林太的湖畔散策、私房茶席與熟齡住宿心法 | 林太的慢活旅行誌」**
...
從 draft.json 讀 `h1` / `topic` / `research`，**規劃 2-4 個 H2 大綱/章節**。

用通用三段式呈現，例：

```
**Step 4 / H2 大綱**
我用「結構與面向」分組，這樣讀者能清晰對比各家規定 ──

## 一、出發前的心情與準備
## 二、各大航空行李規定統整比較
## 三、2026 出國托運最新限制與安全規定

這樣分組大綱可以嗎？
```

意圖分類後:
- A → `h2_outline = [{"h2": "<標題>"}]` + `stage = "h2_done"` → 進 Step 5
- B → 二次確認 → A 動作
...
讀 `h2_outline` / `topic` / `research`，**在每個 H2 大綱底下，細分/展開 2-3 個 H3 小標**(每個 15-25 字，具體不抽象)。

用通用三段式呈現，例：

```
**Step 5 / H3 小標 — 在 H2 底下細分**
我為剛才的每個大綱章節配上具體的內容小標，讓文章結構更豐富 ──

## 一、出發前的心情與準備
   - 傳統航空隨身行李限制：登機箱尺寸與重量上限
   - 台灣虎航隨身行李嚴格秤重與紙板框尺寸測試
## 二、各大航空行李規定統整比較
   - 托運行李大不同：弄懂計件制與計重制的收費差異
   - 華航、長榮、星宇經濟艙免費托運額度與超重費
   - 台灣虎航托運行李預先加購秘訣與現場超規罰金
## 三、2026 出國托運最新限制與安全規定
   - 2026 出國必知：鋰電池與行動電源攜帶安全規定
   - 出發前最後確認：至各航空官網查詢該票種額度

這 7 個小標 OK 嗎？要加要減要換？
```

意圖分類後:
- A → `h3_subheadings = [<list>]` + `h2_outline = [{"h2": "<標題>", "h3_indices": [<idx>, ...]}, ...]` + `stage = "h3_done"` → 進 Step 6
- B → 二次確認 → A 動作
...
FAQ 區塊在文章末尾的命名:**用該人格的口吻命名**(林太用「林太的小叮嚀」,王老闆可能用「王老闆的呷飯心法」)。命名規則寫在 persona.md 裡,從那邊讀(具體位置:「文章架構偏好」段,例如 mrs-lin 寫了「尾段必含:『林太的小叮嚀(FAQ)』」)。沒寫則 fallback 為「<display_name>的問與答」 — 但要避開太技術 / 太冷的字(例:「常見問答」「FAQ」「Q&A」),如果 fallback 出來覺得乾,就主動加一個跟人格氣質一致的修飾詞(例:「林太的問與答」太冷,改成「林太想跟你聊聊」)。

用通用三段式呈現,例:

```
**Step 6 / FAQ(林太的小叮嚀)**
我選了 3 個熟齡讀者最會問的方向:Q1 決策(該不該去 / 怎麼選)、Q2 節奏(時間 / 排程)、Q3 族群顧慮(熟齡視角的具體擔心)──
...
格式:用空行分段,H1 用 `# 標題` markdown 標明,H2 用 `## 章節名` 標明,H3 用 `### 小標` 標明,FAQ 子題用 `**Q1:**` 粗體(避免跟 H3 同層)。

用通用三段式呈現,例:

```
**Step 7 / 全文 — 大約 1850 字**
口吻:林太「我們這個年紀」的體貼語氣;結構:窗邊感觸開場 → 3 個 H2 章節(每章 3 段)→ 哲理收尾 + FAQ。**焦點關鍵字「日月潭慢旅」**在第一段、2 個 H2、結尾各出現一次 ──

# 日月潭兩天一夜慢旅全攻略｜林太的湖畔散策、私房茶席與熟齡住宿心法
...
從 draft 取出 `h1` / `h2_outline` / `h3_subheadings` / `faq` / `full_text` 組裝。HTML 骨架嚴格依 `.agents/skills/persona-writer/SKILL.md` 的 HTML 骨架規範(`<!DOCTYPE html>`、`<head>` 含 title/description/keywords meta、`<body>` 含內容 + 圖片、結尾 JSON-LD)。
...
**存檔**:`write_file` 到 `.agents/skills/persona-writer/personas/<slug>/articles/<YYYYMMDD>-<location>-<topic>.html`(用 deterministic 路徑,沿用 Step 1A-4 的 topic-slug 規則)。
...
執行:
```
python3 .agents/skills/persona-writer/scripts/wp_poster.py <persona-slug> "<H1 標題>" "<已存的 HTML 檔路徑>" draft
```
...
- `os.remove(".agents/skills/persona-writer/personas/<slug>/articles/draft-<...>.json")`
...
1. 用 `read_file` 讀取 `.agents/skills/persona-writer/personas/_template/persona.md` 當骨架
2. 用 `write_file` 建立 `.agents/skills/persona-writer/personas/<新代號>/persona.md`
...
2. 有的話,執行(只 add persona.md,wp-config.json 已經 .gitignored 不會誤入):
   ```
   git add .agents/skills/persona-writer/personas/<新代號>/persona.md
...
用 `read_file` 讀 `.agents/skills/persona-writer/personas/<slug>/persona.md`,把每個欄位用簡短摘要顯示給使用者:
...
2. 執行:
   ```
   git add .agents/skills/persona-writer/personas/<slug>/persona.md
...
| 使用者看到的訊息 | 翻譯 + 解法 |
|---|---|
| `❌ 失敗:401 未授權。Access token 可能已被撤銷` | **wordpress.com 端的鑰匙失效了**。可能使用者去「已連線應用程式」按了 Disconnect,或在 OAuth app 後台按了 Reset Key。重跑授權即可:`python3 .agents/skills/persona-writer/scripts/wp_oauth_setup.py <persona-slug>`,他重新按 Approve 一次就好。 |
...
- **設定檔路徑(per-persona)**:`.agents/skills/persona-writer/personas/<slug>/wp-config.json`
  欄位:`WP_URL`, `WP_USER`, `WP_APP_PWD`
  **每個人格各自有一份,對應一個 WordPress 部落格,沒有共用、沒有覆寫**。
- **設定範例**:`.agents/skills/persona-writer/personas/_template/wp-config.example.json`(placeholder 值)
- **發布腳本**:`.agents/skills/persona-writer/scripts/wp_poster.py`
