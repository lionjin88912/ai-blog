...
> 目標:讓使用者把某個人格的 WordPress 網址、帳號、應用程式密碼填進 `personas/<slug>/wp-config.json`。
> **重要**:**每個人格對應一個 WordPress 部落格,設定獨立不共用**。預設新裝只有林太,所以模組 1 預設帶你設定林太的部落格。
> 想設定其他人格的部落格,直接告訴我「設定王老闆的 WordPress」,流程一樣。
...
跟他說「我等一下會把這三項存進『<人格中文名>』的設定卡,這樣以後她就用這個部落格,可以嗎?」,得到同意後,使用 `write_file` 寫入:
...
寫完後不要等使用者問,直接說「我幫『<人格中文名>』發一篇測試草稿確認連線有沒有通,等我 10 秒」,然後執行(注意第一個參數是該人格的 slug):

```
python3 .gemini/skills/persona-writer/scripts/wp_poster.py <persona-slug> "連線測試 - 可刪除" "<p>這是 Gemini 自動發送的測試文章,確認連線後可以刪除。</p>" draft
```

- 如果看到 `✅ 成功`,告訴使用者「『<人格中文名>』的部落格設定完成!以後直接跟我說『用<人格中文名>寫一篇 XXX』就會發到這個部落格」。
