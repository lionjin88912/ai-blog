...
**Step B2 — WordPress 連線(問 3 題)**

人格設定問完後,接著問:
> 接下來最後 3 題,設定這位人格要發文到的 WordPress 部落格。每位人格對應一個獨立部落格。
>
> 如果你還沒拿到應用程式密碼,先到 WordPress 後台「使用者 → 個人資料 → 應用程式密碼」拿一組(模組 1 有完整教學),拿到了再回來。

依序問(一次一題):

9. **WordPress 網址**(要含 `https://`,例:`https://wang-foodie.example.com`)
10. **WordPress 登入帳號 / Email**
11. **應用程式密碼**(那串 `xxxx xxxx xxxx xxxx xxxx xxxx`)
...
> 密碼:已收到(出於安全不顯示)
>
> 沒問題的話我就幫你建立人格資料夾跟連線設定,還會發一篇測試草稿確認連線有沒有通,要嗎?

**Step D — 建立人格 + WordPress 設定 + 測試發布**
...
3. 用 `write_file` 建立 `.gemini/skills/persona-writer/personas/<新代號>/wp-config.json`,內容:
   ```json
   {
     "WP_URL": "<使用者給的網址>",
     "WP_USER": "<使用者給的帳號>",
   ...
   ```
4. **不要**建立 `published.json`(空著就好,第一次發文時會自動產生)
5. 立刻發測試草稿驗證連線:
   ```
   python3 .gemini/skills/persona-writer/scripts/wp_poster.py <新代號> "連線測試 - 可刪除" "<p>這是 Gemini 自動發送的測試文章,確認連線後可以刪除。</p>" draft
   ```
...
❌ 測試失敗時:
> ⚠️ 人格資料建好了,但 WordPress 連線測試失敗。
>
> 錯誤訊息:<貼出 wp_poster.py 的錯誤輸出>
>
> 可能的原因:應用程式密碼不對 / 網址錯字 / 帳號權限不足。要不要重新跑一次模組 1(指定『<中文名>』)重設?

如果使用者說好試一篇,直接跳到模組 2 用新人格寫一篇。
