...
確認 `.gemini/skills/persona-writer/scripts/wp-config.json` 是否存在。
- **存在**:WordPress 發布功能已就緒,繼續。
- **不存在**:中止流程,告訴使用者「WordPress 還沒設定,請先到 marketing-content-factory 跑模組 1 設定」。**不要在這裡引導設定**,設定流程歸入口層管。
...
├── scripts/
│   ├── wp_poster.py             ← 共用發布腳本
│   ├── wp-config.json           ← 共用 WordPress 連線設定(機密,不可公開)
│   └── wp-config.example.json   ← 設定範例
└── personas/
    ├── _template/
    │   └── persona.md           ← 新人格範本
    ├── mrs-lin-slow-travel/
