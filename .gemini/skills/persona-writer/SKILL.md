...
> 這一階段的詳細流程寫在 `marketing-content-factory` 模組 2 Step 8(包含 8a 找圖規則、8b HTML 組裝、8c SEO 自評項目、8d wp_poster 命令、8e 清檔條件,以及對使用者的回報訊息)。**那邊是 single source of truth,本檔不再重述細節**。persona-writer 只負責按那邊的順序執行,不再對使用者問問題。

執行順序摘要(細節見 Module 2 Step 8):

- **8a 找圖**
- **8b 組完整 HTML**(依下方「🎨 HTML 骨架」規範)
- **8c LLM 自評 SEO**(規範詳見 marketing-content-factory Step 8c)
...
- 8a 圖失敗:不擋發布,繼續 8b 純文字版
- 8c 連 3 次不過:直接 8d
- 8d 發布失敗:`stage` 維持 `full_text_done`,**draft 不刪**,錯誤回報交給 Module 2 對使用者
